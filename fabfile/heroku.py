...
def set_heroku_asset_version(env, git_hash):
    local('heroku config:set ASSET_VERSION={} -a {}'.format(
        git_hash, APP_INFO[env]["heroku_app_name"]))


def set_heroku_maintenance_page(env, url):
    local('heroku config:set MAINTENANCE_PAGE_URL={} -a {}'.format(
        url, APP_INFO[env]["heroku_app_name"]))


def set_heroku_error_page(env, url):
    local('heroku config:set ERROR_PAGE_URL={} -a {}'.format(
        url, APP_INFO[env]["heroku_app_name"]))
...
def deploy(env=None, quick=False):
    ...
    local('pip install -U -r requirements/production.txt')
    ...
    # rebuild_indexes(env=env, asset_version=version, quick=quick)
...
def deploy_source(env=None, asset_version='', quick=False, agency_vars={}):
    ...
    local('git push {} {}:master --force'.format(APP_INFO[env][
...
def clear_cache(env=None, asset_version='', quick=False, agency_vars={}):
    local(
        'heroku run ./manage.py clear_cache -a {}'.format(
            APP_INFO[env]["heroku_app_name"]))


@with_vars
def rebuild_indexes(env=None, asset_version='', quick=False, agency_vars={}):
    local(
        'heroku run ./manage.py rebuild_index --noinput -a {}'.format(
            APP_INFO[env]["heroku_app_name"]))
...
def deploy_static_media(env=None, asset_version='', quick=False, agency_vars={}):
    ...
    print green('Deploying static media {}'.format(
        '__quick__' if quick else ''))
...
def sync_prod_db(env=None, reset_db=False, agency_vars={}):
    ...
    if reset_db:
        ...
        pass
    local(
        'heroku run ./manage.py syncdb -a {}'.format(
            APP_INFO[env]["heroku_app_name"]))
    local(
        'heroku run ./manage.py migrate -a {}'.format(
            APP_INFO[env]["heroku_app_name"]))
